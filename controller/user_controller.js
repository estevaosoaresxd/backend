...
async function updateUserById(req, res) {
    ...
    try {
        ...
        if (parseInt(user.id) != parseInt(userRequisition.id) && userRequisition.isAdmin == false) {
            res.status(500).json(fail('Erro ao atualizar usuário. Não é possivel alterar outro usuário'));
        } else {
            await user.update({ username: username, password: password, isAdmin: user.isAdmin });
...
async function updateUserAdminById(req, res) {
    ...
    try {
        ...
        await user.update({ username: username, password: password, isAdmin: isAdmin });
